...
<div class="main">
	<div class="dangki" style='background:url(img/bg.jpg);'>
		...
		<div class="row">
			<div class="col-xs-12">
				<h3 class="dk">ĐĂNG KÍ THÀNH VIÊN <span class='t1'>ID</span> <span class='t2'>HQSHOP</span></h3>
			</div>
			...
			$checkuser="select * from thanhvienhqshop where tentaikhoan='$user'";
			...
			//check email tồn tại
			$checkemail="select * from thanhvienhqshop where email='$email'";
			...
			if($row_email>0){
				...
				$sqlreg = "INSERT INTO thanhvienhqshop  (tentaikhoan,matkhau,email,ngaysinh,sodienthoai,ngaydangki)";
				$sqlreg .= " VALUES('$user','$pass ','$email','$ngaysinh','$sdt','".date("Y-m-d H:i:s")."')";
				$queryreg=mysqli_query($connection,$sqlreg);
				$sel="select * from thanhvienhqshop where tentaikhoan='$user' and matkhau='$pass'";
	 ...
	 ?>
			<form action="" method="post">
				...
				<div class="col-xs-12">
					...
					<span class="check">Tôi đã đọc và đồng ý với <a href="">Điều Khoản Và Quy Định</a> Của HQSHOP</span>
				</div>
